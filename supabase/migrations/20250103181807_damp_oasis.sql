...
DROP TRIGGER IF EXISTS employee_insert_trigger ON employee_profiles CASCADE;

-- Drop existing functions in correct order
DROP FUNCTION IF EXISTS public.before_employee_insert() CASCADE;
DROP FUNCTION IF EXISTS public.create_auth_user(text, uuid, text) CASCADE;
DROP FUNCTION IF EXISTS public.validate_employee_email(text, uuid, uuid) CASCADE;
...
BEGIN
  ...
  IF existing_user.id IS NOT NULL THEN
    ...
    -- If email exists in auth but not in employee_profiles
    IF NOT EXISTS (
      SELECT 1 FROM employee_profiles 
      WHERE email = p_email 
...
-- Create a function to create auth user with proper error handling
...
EXCEPTION 
  WHEN unique_violation THEN
    -- If we hit a unique violation, the user was created by another concurrent transaction
    SELECT id INTO existing_user_id
    FROM auth.users
    WHERE email = p_email;
...
-- Create trigger function for employee creation
