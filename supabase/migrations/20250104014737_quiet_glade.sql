...
DECLARE
  overlapping_runs record;
  missing_timesheets boolean;
BEGIN
  ...
  IF FOUND THEN
    RETURN jsonb_build_object(
      'valid', false,
      'message', format(
        'Overlapping payroll run exists for period %s to %s (status: %s)',
        overlapping_runs.period_start,
        overlapping_runs.period_end,
        overlapping_runs.status
      )
    );
  END IF;
  ...
  SELECT EXISTS (
    ...
    AND period_end >= p_period_start
    AND status != 'approved'
  ) INTO missing_timesheets;

  IF missing_timesheets THEN
    RETURN jsonb_build_object(
      'valid', false,
      'message', 'All timesheets must be approved before processing payroll'
    );
  END IF;
...
-- Update payroll processing function with validation
