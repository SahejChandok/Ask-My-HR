...
BEGIN
  ...
  -- Create timesheets and entries for each employee
  FOR v_employee_record IN 
    SELECT id, email FROM employee_profiles 
    WHERE tenant_id = v_tenant_id 
    AND id != v_admin_profile_id
  LOOP
    -- Create timesheet
    ...
    INSERT INTO timesheet_entries (
      timesheet_id, date, start_time, end_time, break_minutes, description, is_overtime
    )
    SELECT 
      v_timesheet_id,
      ...
      '09:00'::time,
      CASE 
        WHEN v_employee_record.email = 'jane.smith@example.com' AND n IN (1, 3) THEN '18:00'::time -- Overtime
        ELSE '17:00'::time -- Regular
      END,
      30,
      CASE 
        WHEN v_employee_record.email = 'jane.smith@example.com' AND n IN (1, 3) THEN 'Overtime day'
        ELSE 'Regular day'
      END,
      v_employee_record.email = 'jane.smith@example.com' AND n IN (1, 3) -- Overtime flag
